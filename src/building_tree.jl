...
function build_tree(clusters::Vector{Cluster}, D::Int64, classes;multivariate::Bool=false, time_limit::Int64 = -1, mu::Float64=10^(-4))
    ...
    @objective(m, Max, sum(majority_class_cardinality(clusters[i]) * u_at[i, 1] for i in 1:clusterCount))
...
function majority_class_cardinality(cluster)
    class_counts = countmap(cluster.class)
    majority_class = argmax(class_counts)
    return class_counts[majority_class]
end