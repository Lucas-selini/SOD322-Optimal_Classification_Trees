...
# function newClusteringMethod(X_train,Y_train,gamma)
#     # create DBSCAN clustering object
#     dbscan = DBSCAN(0.1, 0.35,10)
#     # fit the model
#     X_train_df = DataFrame(X_train)
#     fit!(dbscan, Tables.table(X_train_df))
    
#     # get the labels
#     labels = dbscan.assignments
#     # get the number of clusters
#     n_clusters = length(unique(labels))
#     # create clusters
#     clusters = [Cluster([],[],[],[]) for i in 1:n_clusters]
#     # fill the clusters
...
# add a new clustering method different than k means and simple merge
# Define a GMM clustering
function newClusteringMethod(X_train,Y_train,gamma)
    # create GMM clustering object
    gmm = GMM(3, 100,kind=:diag)
    # fit the model
    X_train_df = DataFrame(X_train, :auto)
    em!(gmm, DataOrMatrix(X_train_df), nIter=100, varfloor=1e-6)
    #complete the function to return clusters
    # get the labels
    labels = gmm.assignments
    # get the number of clusters
    n_clusters = length(unique(labels))
    # create clusters
    clusters = [Cluster([],[],[],[]) for i in 1:n_clusters]
